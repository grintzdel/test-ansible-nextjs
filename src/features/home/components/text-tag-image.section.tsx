...
const Card: React.FC<CardProps> = ({title, description, mediaUrl, tags, mediaPosition = 'left'}: CardProps): React.JSX.Element => {
    ...
    return (
        <div className={`flex flex-col p-6 justify-center items-center gap-10 bg-[#D2F8DC] rounded-[20px] max-w-7/8 ${flexDirection} ${alignSelf}`}>
            <div className="flex flex-col gap-17.5">
                <div className="flex flex-col gap-10">
                    <h2 className="text-[#2E4F21] text-[40px] tracking-tight">{title}</h2>
                    <p className="text-[#2E4F21]">{description}</p>
                </div>
                <div className="flex flex-row gap-2.5">
                    {tags.map((tag, index) => (
                        <div key={index} className="bg-white text-[#506349] p-3 rounded-lg text-xs hover:bg-[#506349] hover:text-white">{tag}</div>
                    ))}
                </div>
            </div>
